...
function createSimpsonsStore() {
	return Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url: '../../Concrete/DEBUG_EDITION/GetTaskView/UserId0000/SessionId0000/',
			reader: {
				type: 'json',
				root: 'ArrayModels',
...
function createDictateStore() {
	return Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url: '../../Concrete/DEBUG_EDITION/GetDictatsView/UserId0000/SessionId0000/',
			reader: {
				type: 'json',
				root: 'ArrayModels',
			...
			dockedItems: [{
				...
				items: [{
					...
					handler: function (scope) {
						...
						if (button.text == "新 增") {
							FAST.OnExcuteRequest(editForm.down('form').getForm(), 'form-insert', '../../Concrete/DEBUG_EDITION/', [], '2|新增指令', '新增天文观测指令');
						} else {
							FAST.OnExcuteRequest(editForm.down('form').getForm(), 'form-update', '../../Concrete/DEBUG_EDITION/', [], '2|编辑指令', '编辑天文观测指令');
						}
					}
				}, {
					text: '另存为',
				...
				handler: function (scope) {
					...
					if (form.isValid()) {
						form.submit({
							...
							url: '../../Concrete/DEBUG_EDITION/UpdateModel/12|编辑指令/修改天文观测指令/?parameters={}',
				...
				handler: function (scope) {
					...
					if (form.isValid()) {
						...
						form.submit({
							submitEmptyText: false,
							clientValidation: true,
							url: '../../Concrete/DEBUG_EDITION/AppendDictate/12|新增指令/添加天文观测指令/SessionId0000/?parameters={}',
							success: function (f, operate) {
								QueryDictateData();
							},
							failure: function (f, operate) {
				...
				handler: function (scope) {
					...
					if (form.isValid()) {
						form.submit({
							submitEmptyText: false,
							clientValidation: true,
							url: '../../Concrete/DEBUG_EDITION/SaveAsTask/12|新增指令/添加天文观测指令/?parameters={}',
							success: function (f, operate) {
								HeaderQueryData();
	...
	return Ext.create('Ext.window.Window', {
		...
		buttons: [{
			...
			handler: function (scope) {
				...
				if (form.isValid()) {
					form.submit({
						url: '../../Concrete/DEBUG_EDITION/ImportFile/12|指令导入/导入TXT格式的观测指令集/',
						waitMsg: '处理中，请稍候...',
						method: 'post',
	...
	Ext.MessageBox.alert({
		...
		fn: function (optional) {
			if (optional == "ok") {
				...
				Ext.Ajax.request({
					url: '../../Concrete/DEBUG_EDITION/DeleteTask/12|删除指令/删除天文观测指令/?_dc=' + (new Date().getTime()) + '&parameters={}',
					method: 'POST',
					params: record,
...
function GridCellClick_Dictate(rowIndex) {
	...
	if (current_Sequence.SEQUENCE_STATE == 'DEFAULT') {
		dictate_save.show();
	...
	Ext.MessageBox.alert({
		...
		fn: function (optional) {
			if (optional == "ok") {
				...
				Ext.Ajax.request({
					url: '../../Concrete/DEBUG_EDITION/RemoveDictate/12|移除指令/移除天文观测指令/?_dc=' + (new Date().getTime()) + '&parameters={}',
					method: 'POST',
					params: record,
					success: function (response, options) {
						QueryDictateData();
	...
	Ext.MessageBox.alert({
		...
		fn: function (optional) {
			if (optional == "ok") {
				...
				Ext.Ajax.request({
					url: '../../Concrete/DEBUG_EDITION/DeleteTasks/12|删除指令/删除天文观测指令/?_dc=' + (new Date().getTime()) + '&parameters={}',
					method: 'POST',
					params: record,
					success: function (response, options) {
						FAST.OnInfoEvent('操作处理成功!<br/><br/>');
